...
system('export PATH="/public/home/aibert01/pipeline/software/bin/samtools-1.3.1/bin:$PATH" ')
system('export PATH="/public/home/aibert01/pipeline/software/basesofts/bedtools-2.29.1/bin:$PATH" ')
system('export PATH="/public/home/lit01/pipeline/Population/Immune/lohhla-1.1.6:$PATH" ')
library(optparse)
option_list = list(
  ...
  make_option(c("-hlaLoc", "--HLAfastaLoc"), type="character", default='/public/home/lit01/pipeline/Population/Immune/lohhla-1.1.6/data/abc_complete.fasta', 
  ...
  make_option(c("-no", "--novoDir"), type="character", default='/public/home/lit01/pipeline/Population/Immune/lohhla-1.1.6/novocraft', 
              help="path to novoalign executable [default= %default]", metavar="character"),
  make_option(c("-ga", "--gatkDir"), type="character", default='/public/home/aibert01/pipeline/software/mc3/picard-tools-1.122', 
              help="path to GATK executable [default= %default]", metavar="character"),
  make_option(c("-ex", "--HLAexonLoc"), type="character", default='/public/home/lit01/pipeline/Population/Immune/lohhla-1.1.6/data/hla.dat', 
...
create.kmer.file <- function(workDir, kmerSize, HLAfastaLoc){
  ...
  setwd(workDir)
  cmd <- paste('/public/home/lit01/pipeline/Population/Immune/lohhla-1.1.6/jellyfish-linux count -m ', kmerSize, ' -s 100M -t 5 ', HLAfastaLoc, sep = '')
  print(cmd)
  system(cmd)
  cmd <- paste('/public/home/lit01/pipeline/Population/Immune/lohhla-1.1.6/jellyfish-linux dump mer_counts.jf > mer_counts_dumps.fa', sep = '')
  print(cmd)
  system(cmd)
...
)
{
  ...
  if(!override){
    ...
    ## never will include paths to BAMs for Nextflow
    for(BAM in BAMs){
      ## region <- unlist(strsplit(BAM, split = '/'))[length(unlist(strsplit(BAM, split = '/')))]
      ## remove *.bam
      region = basename(gsub(".bam", "", BAM)) ###lit modify
      cmd    <- paste('samtools flagstat ', BAM, ' > ', outDir, region, '.proc.flagstat', sep = '')
      print(cmd)
      system(cmd)
    }  
...
regions   <- sapply(BAMfiles, FUN =function(x) {return(unlist(strsplit(basename(x), split = '.bam'))[1])})   ## contains both tumor and normal now
...
if(mapping.step){
    ...
    for(BAMfile in BAMfiles){
        ...
        BAMid <- unlist(strsplit(basename(BAMfile), split = '.bam'))[1] ###lit modify
...
for (region in regions){
    ...
    if(paste(region, ".bam", sep = "") == basename(normalBAMfile)){
        type <- "normal"
    } else{
        type <- "tumor"
    }
    ...
    for (BAMfile in BAMfiles){
        ...
        mpileupFile <- paste(workDir, '/', region,".",hlaAllele,".",type,".mpileup",sep="") ###lit modify
    
        print(paste(workDir, '/',region,".",hlaAllele,".",type,".mpileup",sep=""))
...
if(runWithNormal){
  ...
  normalName = gsub(".bam", "", basename(normalBAMfile))
    ...
    if(coverageStep)
    {
      ...
      for (HLA_gene in c('hla_a','hla_b','hla_c'))
      {
        ...
        #HLAtype 1 coverage
        {
          ...
          print('line1057----------------HLA_A_type1tumorCov')
        }
		...
		print(HLA_A_type2)
        {
		  ...
		  print(HLA_A_type2normalLoc)
		  print('line 1063 HLA_A_type2normalLoc')
		  f=read.table(HLA_A_type2normalLoc ,sep="\t",stringsAsFactors=FALSE,quote="",fill=TRUE)
          if(runWithNormal){
            HLA_A_type2normal <- read.table(HLA_A_type2normalLoc ,sep="\t",stringsAsFactors=FALSE,quote="",fill=TRUE)
			print('HLA_A_type2normal')
			print('---------------++++++++++++HLA_A_type2normal')
          }
		  print('HLA_A_type2normal')
		  print(HLA_A_type2normal)
          ...
          if (region != normalName){
		    print(paste(workDir, "/",region,".",HLA_A_type2,".","tumor.mpileup",sep=""))
     ...
     print('1115----1115----1115---1115---1115') 
